	...
	\begin{document}
		...
		\end{schema}
  		...
  		\begin{schema}{ReplaceSensorOK}
			\Delta TempMonitor\\
			sensorToReplace? : SENSOR\_TYPE\\
			newSensor? : LOCATION\_TYPE\\
			\where
			sensor? \in deployed\\
			location? \notin \ran map\\
			deployed' = deployed \cup \{ newSensor? \} \char`\\ \{sensorToReplace? \} \\
			map' = map \cup \{ newSensor? \mapsto map(sensorToReplace?) \} \dsub \{sensorToReplace\}\\
			read' = read \cup \{ newSensor? \mapsto read(sensorToReplace?) \} \dsub \{sensorToReplace\}
		\end{schema}
